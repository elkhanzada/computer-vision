function [ncc_scores,ssd_scores] = rotate_descriptor(path_1,path_2,vec1,vec2,n,image_xx_1,image_xy_1,image_yy_1,image_xx_2,image_xy_2,image_yy_2)
    ...
    for i=1:size(x1,1)
        ...
        y = y1(i);
        [h,w] = size(image_1);
        [left_row,right_row,left_col,right_col] = index_checker(x-2*n,x+2*n,y-2*n,y+2*n,h,w);
        patch_xx_1 = image_xx_1(x-n:x+n,y-n:y+n);
        patch_xy_1 = image_xy_1(x-n:x+n,y-n:y+n);
        patch_yy_1 = image_yy_1(x-n:x+n,y-n:y+n);
        ...
        for j=1:size(x2,1)
            ...
            [h,w] = size(image_2);
            [left_row,right_row,left_col,right_col] = index_checker(x-2*n,x+2*n,y-2*n,y+2*n,h,w);
            patch_xx_2 = image_xx_2(x-n:x+n,y-n:y+n);
            patch_xy_2 = image_xy_2(x-n:x+n,y-n:y+n);
            patch_yy_2 = image_yy_2(x-n:x+n,y-n:y+n);
